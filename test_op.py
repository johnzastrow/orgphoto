...
class TestIntegration(unittest.TestCase):
    ...
    def create_test_image(self, path: Path, content: bytes = None, create_dirs: bool = True):
...
class TestDuplicateModes(unittest.TestCase):
    ...
    def test_rename_duplicate_mode(self):
        ...
        date_dir = self.dest_dir / "2023_05_15"
        existing_file = self.create_test_image(date_dir / "photo.jpg", b"existing content")
        
        # Create source file with same name but different content
        source_file = self.create_test_image(self.source_dir / "photo.jpg", b"new content")
    ...
    def test_redirect_duplicate_mode(self):
        ...
        source_file = self.create_test_image(self.source_dir / "photo.jpg", b"duplicate content")
        ...
        with patch('op.get_created_date') as mock_date:
            ...
            args = [
                '-c',  # copy mode
                '-D', 'redirect',  # redirect duplicates
                '-j', 'jpg',
                str(self.source_dir),
                str(self.dest_dir)
            ]
